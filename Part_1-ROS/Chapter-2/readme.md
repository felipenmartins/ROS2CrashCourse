...
Let’s focus on the second line, which corresponds to actions (the arrow keys publish values to the `cmd_vel` topic).

Notice that the letter keys `G|B|V|C|D|E|R|T` form a “box” around the `F` key on a US QWERTY keyboard. Each key’s position around `F` corresponds to an absolute desired orientation for the turtle in TurtleSim. For example, `R` indicates that the desired orientation of the turtle is facing up, `E` corresponds to 45 degrees to the left, `V` is facing down etc.. Every time you press one of those keys, you are sending a goal to an action server that is part of the `/turtlesim` node with an indication of the desired orientation. In this case, the goal is to orient the turtle accordingly, which will result on a rotation of the turtle around its own axis. A message relaying the result of the goal should display once the turtle completes its rotation:
```
...
#### Inspecting Nodes
To see the available actions of the `/turtlesim` node, open a new terminal and run the command:
```bash
ros2 node info /turtlesim
```

It will return a list of subscribers, publishers, services, action servers and action clients:
```
    /turtlesim
      Subscribers:
        /parameter_events: rcl_interfaces/msg/ParameterEvent
...
```
    ...
    Action servers: 1
        /turtlesim
```
This gives us the same information we learned before from `ros2 node info`, but focused on the actions only: the `/teleop_turtle` node has an action client and the `/turtlesim` node has an action server for the `/turtle1/rotate_absolute` action.
...
```
    ...
    float32 remaining
```
The first section of this message, before the first ``---``, corresponds to the structure of the goal request (data type and name). The following section describes the structure of the result, and the last one shows the structure of the feedback.
...
#### Practice on the iRobot Create3
